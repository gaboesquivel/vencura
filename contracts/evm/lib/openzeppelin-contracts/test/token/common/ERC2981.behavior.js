...
function shouldBehaveLikeERC2981() {
  ...
  describe('default royalty', () => {
    ...
    it('checks royalty is set', async function () {
      expect(
        await this.token.royaltyInfo(this.tokenId1, this.salePrice),
      ).to.deep.equal([
    ...
    it('Remove royalty information', async function () {
      ...
      expect(
        await this.token.royaltyInfo(this.tokenId1, this.salePrice),
      ).to.deep.equal([ethers.ZeroAddress, newValue])
    ...
    it('reverts if invalid parameters', async function () {
      ...
      await expect(
        this.token.$_setDefaultRoyalty(ethers.ZeroAddress, royaltyFraction),
      )
  ...
  describe('token based royalty', () => {
    beforeEach(async function () {
      await this.token.$_setTokenRoyalty(
        this.tokenId1,
        this.account1,
        royaltyFraction,
      )
    ...
    it('can reset token after setting royalty', async function () {
      ...
      expect(
        await this.token.royaltyInfo(this.tokenId2, this.salePrice),
      ).to.deep.equal([this.account1.address, 0n])
    })
