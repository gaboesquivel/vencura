...
function shouldBehaveLikeERC20(initialSupply, opts = {}) {
  ...
  describe('transfer from', () => {
    describe('when the token owner is not the zero address', () => {
      describe('when the recipient is not the zero address', () => {
        describe('when the spender has enough allowance', () => {
          beforeEach(async function () {
            await this.token
              .connect(this.holder)
              .approve(this.recipient, initialSupply)
          })
        ...
        describe('when the spender does not have enough allowance', () => {
          ...
          beforeEach(async function () {
            await this.token
              .connect(this.holder)
              .approve(this.recipient, allowance)
          })
        ...
        describe('when the spender has unlimited allowance', () => {
          beforeEach(async function () {
            await this.token
              .connect(this.holder)
              .approve(this.recipient, ethers.MaxUint256)
            this.tx = await this.token
              .connect(this.recipient)
      ...
      it('reverts when the recipient is the zero address', async function () {
        ...
        await expect(
          this.token
            .connect(this.recipient)
            .transferFrom(this.holder, ethers.ZeroAddress, value),
        )
    ...
    it('reverts when the token owner is the zero address', async function () {
      ...
      //   is address(0)
      const errorName = this.token.temporaryApprove
        ? 'ERC20InvalidSender'
      ...
      await expect(
        this.token
          .connect(this.recipient)
          .transferFrom(ethers.ZeroAddress, this.recipient, value),
      )
  ...
  describe('approve', () => {
    beforeEach(function () {
      this.approve = (owner, spender, value) =>
