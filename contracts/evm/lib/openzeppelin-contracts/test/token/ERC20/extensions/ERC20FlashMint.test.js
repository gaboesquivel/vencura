...
async function fixture() {
  ...
  const token = await ethers.deployContract('$ERC20FlashMintMock', [
    name,
    symbol,
  ])
...
describe('ERC20FlashMint', () => {
  ...
  describe('maxFlashLoan', () => {
    it('token match', async function () {
      expect(await this.token.maxFlashLoan(this.token)).to.equal(
        ethers.MaxUint256 - initialSupply,
      )
    })
  ...
  describe('flashLoan', () => {
    it('success', async function () {
      const receiver = await ethers.deployContract('ERC3156FlashBorrowerMock', [
        true,
        true,
      ])
    ...
    describe('custom flash fee & custom fee receiver', () => {
      ...
      beforeEach('init receiver balance & set flash fee', async function () {
        ...
        expect(await this.token.flashFee(this.token, loanValue)).to.equal(
          flashFee,
        )
      })
      ...
      it('default flash fee receiver', async function () {
        ...
        await expect(tx)
          ...
          .withArgs(
            this.token,
            this.receiver,
            receiverInitialBalance + loanValue,
          )
          .to.emit(this.receiver, 'TotalSupply')
          .withArgs(
            this.token,
            initialSupply + receiverInitialBalance + loanValue,
          )
        ...
        expect(await this.token.allowance(this.receiver, this.token)).to.equal(
          0n,
        )
      })
      ...
      it('custom flash fee receiver', async function () {
        ...
        expect(await this.token.$_flashFeeReceiver()).to.equal(
        ...
        expect(await this.token.totalSupply()).to.equal(
          initialSupply + receiverInitialBalance,
        )
        expect(
          await this.token.allowance(this.receiver, flashFeeReceiverAddress),
        ).to.equal(0n)
      })
    })
  })
