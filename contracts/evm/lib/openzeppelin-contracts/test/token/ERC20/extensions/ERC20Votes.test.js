  ...
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`vote with ${mode}`, () => {
      ...
      it('recent checkpoints', async function () {
        ...
        // recent
        expect(
          await this.token.getPastVotes(this.holder, timepoint - 1n),
        ).to.equal(5n)
        // non-recent
        expect(
          await this.token.getPastVotes(this.holder, timepoint - 6n),
        ).to.equal(0n)
      })
      ...
      describe('set delegation', () => {
        describe('call', () => {
          it('delegation with balance', async function () {
            ...
            expect(await this.token.delegates(this.holder)).to.equal(
              ethers.ZeroAddress,
            )

            const tx = await this.token
              .connect(this.holder)
              .delegate(this.holder)
            ...
            await mine()
            expect(
              await this.token.getPastVotes(this.holder, timepoint),
            ).to.equal(supply)
          })
        ...
        describe('with signature', () => {
          ...
          it('rejects bad delegatee', async function () {
            ...
            const { args } = await tx
              ...
              .then((receipt) =>
                receipt.logs.find(
                  (event) => event.fragment.name == 'DelegateChanged',
                ),
              )
          ...
          it('rejects expired permit', async function () {
            const expiry =
              (await time.clock.timestamp()) - time.duration.weeks(1)
            ...
            await expect(
              this.token.delegateBySig(this.holder, nonce, expiry, v, r, s),
            )
      ...
      describe('change delegation', () => {
        ...
        it('call', async function () {
          ...
          expect(
            await this.token.getPastVotes(this.delegatee, timepoint - 1n),
          ).to.equal(0n)
          await mine()
          expect(
            await this.token.getPastVotes(this.holder, timepoint),
          ...
          expect(
            await this.token.getPastVotes(this.delegatee, timepoint),
          ).to.equal(supply)
        })
      })
      ...
      describe('transfers', () => {
        ...
        it('no delegation', async function () {
          await expect(
            this.token.connect(this.holder).transfer(this.recipient, 1n),
          )
        ...
        afterEach(async function () {
          expect(await this.token.getVotes(this.holder)).to.equal(
            this.holderVotes,
          )
          expect(await this.token.getVotes(this.recipient)).to.equal(
            this.recipientVotes,
          )
          ...
          expect(
            await this.token.getPastVotes(this.recipient, timepoint),
      ...
      describe('Compound test suite', () => {
        ...
        describe('numCheckpoints', () => {
          it('returns the number of checkpoints for a delegate', async function () {
            ...
            await mine()
            expect(
              await this.token.getPastVotes(this.other1, t1.timepoint),
            ).to.equal(100n)
            expect(
              await this.token.getPastVotes(this.other1, t2.timepoint),
            ).to.equal(90n)
            expect(
              await this.token.getPastVotes(this.other1, t3.timepoint),
            ).to.equal(80n)
            expect(
              await this.token.getPastVotes(this.other1, t4.timepoint),
            ).to.equal(100n)
          })
          ...
          it('does not add more than one checkpoint in a block', async function () {
            ...
            const [t1, t2, t3] = await batchInBlock([
              () =>
                this.token
                  .connect(this.recipient)
                  .delegate(this.other1, { gasLimit: 200000 }),
              () =>
                this.token
                  .connect(this.recipient)
                  .transfer(this.other2, 10n, { gasLimit: 200000 }),
              () =>
                this.token
                  .connect(this.recipient)
                  .transfer(this.other2, 10n, { gasLimit: 200000 }),
            ...
            expect(
              await this.token.checkpoints(this.other1, 0n),
            ).to.be.deep.equal([t1.timepoint, 80n])

            const t4 = await this.token
              .connect(this.holder)
              .transfer(this.recipient, 20n)
            t4.timepoint = await time.clockFromReceipt[mode](t4)

            expect(await this.token.numCheckpoints(this.other1)).to.equal(2n)
            expect(
              await this.token.checkpoints(this.other1, 1n),
            ).to.be.deep.equal([t4.timepoint, 100n])
          })
        })
        ...
        describe('getPastVotes', () => {
          ...
          it('returns the latest block if >= last checkpoint block', async function () {
            ...
            expect(
              await this.token.getPastVotes(this.other1, timepoint),
            ).to.equal(supply)
            expect(
              await this.token.getPastVotes(this.other1, timepoint + 1n),
            ).to.equal(supply)
          })
          ...
          it('generally returns the voting balance at the appropriate checkpoint', async function () {
            ...
            expect(
              await this.token.getPastVotes(this.other1, t1.timepoint - 1n),
            ).to.equal(0n)
            expect(
              await this.token.getPastVotes(this.other1, t1.timepoint),
            ).to.equal(supply)
            ...
            expect(
              await this.token.getPastVotes(this.other1, t2.timepoint + 1n),
            ).to.equal(supply - 10n)
            expect(
              await this.token.getPastVotes(this.other1, t3.timepoint),
            ).to.equal(supply - 20n)
            expect(
              await this.token.getPastVotes(this.other1, t3.timepoint + 1n),
            ).to.equal(supply - 20n)
            expect(
              await this.token.getPastVotes(this.other1, t4.timepoint),
            ).to.equal(supply)
            expect(
              await this.token.getPastVotes(this.other1, t4.timepoint + 1n),
            ).to.equal(supply)
          })
        })
      })
      ...
      describe('getPastTotalSupply', () => {
        ...
        it('returns the latest block if >= last checkpoint block', async function () {
          ...
          expect(await this.token.getPastTotalSupply(timepoint)).to.equal(
            supply,
          )
          expect(await this.token.getPastTotalSupply(timepoint + 1n)).to.equal(
            supply,
          )
        })
        ...
        it('generally returns the voting balance at the appropriate checkpoint', async function () {
          ...
          expect(
            await this.token.getPastTotalSupply(t1.timepoint - 1n),
          ).to.equal(0n)
          expect(await this.token.getPastTotalSupply(t1.timepoint)).to.equal(
            supply,
          )
          expect(
            await this.token.getPastTotalSupply(t1.timepoint + 1n),
          ).to.equal(supply)
          expect(await this.token.getPastTotalSupply(t2.timepoint)).to.equal(
            supply - 10n,
          )
          expect(
            await this.token.getPastTotalSupply(t2.timepoint + 1n),
          ).to.equal(supply - 10n)
          expect(await this.token.getPastTotalSupply(t3.timepoint)).to.equal(
            supply - 20n,
          )
          expect(
            await this.token.getPastTotalSupply(t3.timepoint + 1n),
          ).to.equal(supply - 20n)
          expect(await this.token.getPastTotalSupply(t4.timepoint)).to.equal(
            supply,
          )
          expect(
            await this.token.getPastTotalSupply(t4.timepoint + 1n),
          ).to.equal(supply)
        })
      })
    })
