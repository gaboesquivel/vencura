...
describe('ShortStrings', () => {
  ...
  for (const str of [0, 1, 16, 31, 32, 64, 1024].map((length) =>
    'a'.repeat(length),
  )) {
      ...
      it('set / get with fallback', async function () {
        const short = await this.mock
          ...
          .then(
            (receipt) =>
              receipt.logs.find(
                (ev) => ev.fragment.name == 'return$toShortStringWithFallback',
              ).args[0],
          )
        ...
        expect(await this.mock.$byteLengthWithFallback(short, 0)).to.equal(
          str.length,
        )
