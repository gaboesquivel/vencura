...
describe('Packing', () => {
  ...
  describe('pack', () => {
    for (const [size1, size2] of product(SIZES, SIZES).filter(
      ([size1, size2]) => SIZES.includes(size1 + size2),
    )) {
      ...
      it(`pack bytes${size1} + bytes${size2} => bytes${size1 + size2}`, async function () {
        expect(
          await this.mock[`$pack_${size1}_${size2}`](value1, value2),
        ).to.equal(packed)
        expect(
          await this.mock[`$extract_${size1 + size2}_${size1}`](packed, 0),
        ).to.equal(value1)
        expect(
          await this.mock[`$extract_${size1 + size2}_${size2}`](packed, size1),
        ).to.equal(value2)
      })
    }
  })
  ...
  describe('extract / replace', () => {
    for (const [size1, size2] of product(SIZES, SIZES).filter(
      ...
      it(`extract bytes${size2} from bytes${size1}`, async function () {
        expect(
          await this.mock[`$extract_${size1}_${size2}`](outer, offset),
        ).to.equal(ethers.hexlify(outer.slice(offset, offset + size2)))
      ...
      it(`replace bytes${size2} from bytes${size1}`, async function () {
        expect(
          await this.mock[`$replace_${size1}_${size2}`](outer, value, offset),
        ).to.equal(
          ethers.concat([
            outer.slice(0, offset),
            value,
            outer.slice(offset + size2),
          ]),
        )
