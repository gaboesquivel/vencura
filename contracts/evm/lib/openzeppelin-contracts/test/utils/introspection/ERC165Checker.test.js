...
describe('ERC165Checker', () => {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture))
  })

  describe('ERC165 missing return data', () => {
    ...
    it('does not support mock interface via supportsInterface', async function () {
      expect(await this.mock.$supportsInterface(this.target, DUMMY_ID)).to.be
        .false
    })

    it('does not support mock interface via supportsAllInterfaces', async function () {
      expect(await this.mock.$supportsAllInterfaces(this.target, [DUMMY_ID])).to
        .be.false
    })

    it('does not support mock interface via getSupportedInterfaces', async function () {
      expect(
        await this.mock.$getSupportedInterfaces(this.target, [DUMMY_ID]),
      ).to.deep.equal([false])
    })

    it('does not support mock interface via supportsERC165InterfaceUnchecked', async function () {
      expect(
        await this.mock.$supportsERC165InterfaceUnchecked(
          this.target,
          DUMMY_ID,
        ),
      ).to.be.false
  ...
  describe('ERC165 malicious return data', () => {
    beforeEach(async function () {
  ...
  describe('ERC165 and single interface supported', () => {
    beforeEach(async function () {
      this.target = await ethers.deployContract('ERC165InterfacesSupported', [
        [DUMMY_ID],
      ])
  ...
  describe('ERC165 and many interfaces supported', () => {
    ...
    it('supports each interfaceId via supportsInterface', async function () {
      for (const interfaceId of supportedInterfaces) {
        expect(await this.mock.$supportsInterface(this.target, interfaceId)).to
          .be.true
      }
    })
    ...
    it('supports all interfaceIds via supportsAllInterfaces', async function () {
      expect(
        await this.mock.$supportsAllInterfaces(
          this.target,
          supportedInterfaces,
        ),
      ).to.be.true
    ...
    it('supports none of the interfaces queried via supportsAllInterfaces', async function () {
      ...
      expect(
        await this.mock.$supportsAllInterfaces(this.target, interfaceIdsToTest),
      ).to.be.false
    })
    ...
    it('supports all interfaceIds via getSupportedInterfaces', async function () {
      expect(
        await this.mock.$getSupportedInterfaces(
          this.target,
          supportedInterfaces,
        ),
      ).to.deep.equal(
  ...
  describe('account address does not support ERC165', () => {
    it('does not support ERC165', async function () {
      expect(await this.mock.$supportsERC165(DUMMY_ACCOUNT)).to.be.false
    })

    it('does not support mock interface via supportsInterface', async function () {
      expect(await this.mock.$supportsInterface(DUMMY_ACCOUNT, DUMMY_ID)).to.be
        .false
    })

    it('does not support mock interface via supportsAllInterfaces', async function () {
      expect(await this.mock.$supportsAllInterfaces(DUMMY_ACCOUNT, [DUMMY_ID]))
        .to.be.false
    })

    it('does not support mock interface via getSupportedInterfaces', async function () {
      expect(
        await this.mock.$getSupportedInterfaces(DUMMY_ACCOUNT, [DUMMY_ID]),
      ).to.deep.equal([false])
    })
