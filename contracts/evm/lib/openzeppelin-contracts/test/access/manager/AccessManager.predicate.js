...
const LIKE_COMMON_IS_EXECUTING = {
  executing() {
    it('succeeds', async function () {
      await this.caller.sendTransaction({
        to: this.target,
        data: this.calldata,
      })
    })
...
function testAsClosable({ closed, open }) {
  describe('when the manager is closed', () => {
  ...
  describe('when the manager is open', () => {
...
function testAsRestrictedOperation({
  ...
  describe('when the call comes from the manager (msg.sender == manager)', () => {
...
function testAsDelayedOperation() {
  ...
  describe('without operation delay', () => {
    beforeEach('set operation delay', async function () {
...
function testAsHasRole({ publicRoleIsRequired, specificRoleIsRequired }) {
  describe('when the function requires the caller to be granted with the PUBLIC_ROLE', () => {
  ...
  describe('when the function requires the caller to be granted with a role other than PUBLIC_ROLE', () => {
    beforeEach('set target function role as PUBLIC_ROLE', async function () {
      await this.manager
        .connect(this.roles.ADMIN.members[0])
        .$_setTargetFunctionRole(
...
function testAsGetAccess({
  ...
  describe('when the required role is granted to the caller', () => {
    describe('when role granting is delayed', () => {
      beforeEach('define delay', function () {
        this.grantDelay = time.duration.minutes(3)
        this.delay = this.grantDelay // For testAsDelay
      })

      describe('when caller has an execution delay', () => {
      ...
      describe('when caller has no execution delay', () => {
        beforeEach('set role and delay', async function () {
    ...
    describe('when role granting is not delayed', () => {
      beforeEach('define delay', function () {
        this.grantDelay = 0n
      })
  ...
  describe('when role is not granted', () => {
