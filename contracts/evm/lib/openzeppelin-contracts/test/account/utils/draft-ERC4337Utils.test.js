...
describe('ERC4337Utils', () => {
  ...
  describe('parseValidationData', () => {
    it('parses the validation data', async function () {
      ...
      const validationData = packValidationData(
        validAfter,
        validUntil,
        authorizer,
      )
      ...
      expect(
        this.utils.$parseValidationData(validationData),
      ).to.eventually.deep.equal([authorizer.address, validAfter, validUntil])
    })
    ...
    it('returns an type(uint48).max if until is 0', async function () {
      ...
      expect(
        this.utils.$parseValidationData(validationData),
      ).to.eventually.deep.equal([authorizer.address, validAfter, MAX_UINT48])
    })

    it('parse canonical values', async function () {
      expect(
        this.utils.$parseValidationData(this.SIG_VALIDATION_SUCCESS),
      ).to.eventually.deep.equal([ethers.ZeroAddress, 0n, MAX_UINT48])

      expect(
        this.utils.$parseValidationData(this.SIG_VALIDATION_FAILED),
      ).to.eventually.deep.equal([ADDRESS_ONE, 0n, MAX_UINT48])
    })
  })

  describe('packValidationData', () => {
    ...
    it('packing reproduced canonical values', async function () {
      expect(
        this.utils.$packValidationData(
          ethers.Typed.address(ethers.ZeroAddress),
          0n,
          0n,
        ),
      ).to.eventually.equal(this.SIG_VALIDATION_SUCCESS)
      expect(
        this.utils.$packValidationData(ethers.Typed.bool(true), 0n, 0n),
      ).to.eventually.equal(this.SIG_VALIDATION_SUCCESS)
      expect(
        this.utils.$packValidationData(
          ethers.Typed.address(ADDRESS_ONE),
          0n,
          0n,
        ),
      ).to.eventually.equal(this.SIG_VALIDATION_FAILED)
      expect(
        this.utils.$packValidationData(ethers.Typed.bool(false), 0n, 0n),
      ).to.eventually.equal(this.SIG_VALIDATION_FAILED)
    })
  })

  describe('combineValidationData', () => {
  ...
  describe('getValidationData', () => {
    it('returns the validation data with valid validity range', async function () {
      ...
      expect(
        this.utils.$getValidationData(validationData),
      ).to.eventually.deep.equal([aggregator.address, false])
    })
    ...
    it('returns the validation data with invalid validity range (expired)', async function () {
      ...
      expect(
        this.utils.$getValidationData(validationData),
      ).to.eventually.deep.equal([aggregator.address, true])
    })
  ...
  describe('hash', () => {
    it('returns the operation hash with specified entrypoint and chainId', async function () {
      ...
      expect(
        this.utils.$hash(userOp.packed, this.entrypoint, chainId),
      ).to.eventually.equal(userOp.hash(this.entrypoint, chainId))
      expect(
        this.utils.$hash(userOp.packed, this.entrypoint, otherChainId),
      ).to.eventually.equal(userOp.hash(this.entrypoint, otherChainId))
    })
  })

  describe('userOp values', () => {
    describe('intiCode', () => {
      beforeEach(async function () {
      ...
      it('returns factory', async function () {
        expect(this.utils.$factory(this.userOp.packed)).to.eventually.equal(
          this.factory,
        )
        expect(
          this.utils.$factory(this.emptyUserOp.packed),
        ).to.eventually.equal(ethers.ZeroAddress)
      })

      it('returns factoryData', async function () {
        expect(this.utils.$factoryData(this.userOp.packed)).to.eventually.equal(
          '0x123456',
        )
        expect(
          this.utils.$factoryData(this.emptyUserOp.packed),
    ...
    it('returns verificationGasLimit', async function () {
      ...
      expect(
        this.utils.$verificationGasLimit(userOp.packed),
      ).to.eventually.equal(userOp.verificationGas)
    })
    ...
    it('returns callGasLimit', async function () {
      ...
      expect(this.utils.$callGasLimit(userOp.packed)).to.eventually.equal(
        userOp.callGas,
      )
    })
    ...
    it('returns maxPriorityFeePerGas', async function () {
      ...
      expect(
        this.utils.$maxPriorityFeePerGas(userOp.packed),
      ).to.eventually.equal(userOp.maxPriorityFee)
    })
    ...
    it('returns maxFeePerGas', async function () {
      ...
      expect(this.utils.$maxFeePerGas(userOp.packed)).to.eventually.equal(
        userOp.maxFeePerGas,
      )
    })
    ...
    it('returns gasPrice', async function () {
      ...
      expect(this.utils.$gasPrice(userOp.packed)).to.eventually.equal(
    ...
    describe('paymasterAndData', () => {
      ...
      it('returns paymaster', async function () {
        ...
        expect(
          this.utils.$paymaster(this.emptyUserOp.packed),
        ).to.eventually.equal(ethers.ZeroAddress)
      })

      it('returns verificationGasLimit', async function () {
        expect(
          this.utils.$paymasterVerificationGasLimit(this.userOp.packed),
        ).to.eventually.equal(this.userOp.paymasterVerificationGasLimit)
        expect(
      ...
      it('returns postOpGasLimit', async function () {
        expect(
          this.utils.$paymasterPostOpGasLimit(this.userOp.packed),
        ).to.eventually.equal(this.userOp.paymasterPostOpGasLimit)
        expect(
          this.utils.$paymasterPostOpGasLimit(this.emptyUserOp.packed),
        ).to.eventually.equal(0n)
      })

      it('returns data', async function () {
        expect(
          this.utils.$paymasterData(this.userOp.packed),
        ).to.eventually.equal(this.userOp.paymasterData)
        expect(
          this.utils.$paymasterData(this.emptyUserOp.packed),
        ).to.eventually.equal('0x')
      })
    })
  })
