...
const clockFromReceipt = {
  blocknumber: (receipt) =>
    Promise.resolve(ethers.toBigInt(receipt.blockNumber)),
  timestamp: (receipt) =>
    ethers.provider
      .getBlock(receipt.blockNumber)
      .then((block) => ethers.toBigInt(block.timestamp)),
}
const increaseBy = {
  blockNumber: mine,
  timestamp: (delay, mine = true) =>
    time
      .latest()
      .then((clock) =>
        increaseTo.timestamp(clock + ethers.toNumber(delay), mine),
      ),
}
const increaseTo = {
...
const duration = mapValues(
  time.duration,
  (fn) => (n) => ethers.toBigInt(fn(ethers.toNumber(n))),
)
