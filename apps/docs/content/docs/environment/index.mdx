...
**Server Apps (API, Elysia):**
- Use `validateEnvOrThrow` from `@vencura/lib`
- Export `zEnv` as const object: `export const zEnv = getEnv()`
- Server crashes on invalid config (fails fast)
...
7. **Always use zEnv** - Import `zEnv` from `lib/env.ts` instead of accessing `process.env` directly
8. **Leverage @vencura/lib** - Use `validateEnvOrThrow` or `getEnvHelper` - no custom validation logic
9. **Zod schemas** - Use `z.coerce.number()`, `z.optional()`, `z.default()` for proper type handling
