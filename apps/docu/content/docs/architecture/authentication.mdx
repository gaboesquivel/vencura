...
### Why `POST /auth/session/logout` exists alongside Dynamic

[Dynamic](https://www.dynamic.xyz/) owns **its** identity product and SDK/browser state. The API still ships logout because Vencura keeps **separate** session and credential concerns:

- **Postgres `sessions`** — Logout deletes the row for the current session id when it exists. Dynamic does not manage that table.
- **Bearer JWTs** — A JWT remains valid until expiry unless you maintain a denylist (this API does not). Logout does not invalidate Dynamic JWKS. Clients must stop sending the token and clear storage; use the Dynamic SDK where you need to clear **their** UI/session state.
- **API keys** — `venc_…` keys are not revoked by session logout; use `DELETE /account/apikeys/:id`.
- **Stable contract** — Web and OpenAPI expose one “end session” call. For principals that never had a local row (for example some Dynamic-only API Reference flows), the delete may affect **zero** rows; clients should still discard tokens.
