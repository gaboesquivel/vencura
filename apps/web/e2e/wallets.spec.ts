...
test.describe
  .skip('Wallets Dashboard (Dynamic auth E2E paused)', () => {
    test.setTimeout(30_000)

    test('should show wallets page and create wallet', async ({ authenticatedPage: page }) => {
      await page.goto('/wallets')
      await expect(page.locator('h1:has-text("Custodial Wallets")')).toBeVisible({
        timeout: 15_000,
      })
    ...
    test('should show error when sending to invalid address', async ({
      authenticatedPage: page,
    }) => {
      await page.goto('/wallets')
      await expect(page.locator('h1:has-text("Custodial Wallets")')).toBeVisible({
